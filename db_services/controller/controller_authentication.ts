...
export async function generateJwt(req:any, res: any): Promise<any>{
    try{
        const authentication_services_host: string = process.env.AUTHENTICATION_SERVICES_HOST || '';
        const authentication_services_port: number = parseInt(process.env.AUTHENTICATION_SERVICES_PORT as string) || 0;
